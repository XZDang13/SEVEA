...
class Trainer:
    def __init__(self, task_name:str, seed:int, visual:bool=False):
        ...
        if not np.all(np.isfinite(action_low)) or not np.all(np.isfinite(action_high)):
            raise ValueError("PPO requires finite action bounds.")
        if not np.allclose(action_low, -action_high, atol=1e-6):
            raise ValueError(
                f"PPO squashed Gaussian expects symmetric action bounds, got low={action_low}, high={action_high}"
            )
    ...
    def rollout(self):
        ...
        for i in range(self.max_steps):
            ...
            next_obs, reward, terminated, truncated, info = self.train_envs.step(action.cpu().numpy())
    ...
    def eval(self):
        ...
        for i in range(self.eval_steps):
            ...
            next_obs, reward, _, _, info = self.eval_envs.step(action.cpu().numpy())
            next_obs = self._select_observation(next_obs)
            if "success" in info:
